...
const data: QAs[] = [
  {
    id: 1,
    title: `Unicode 是__里的一项业界标准，包括__和__等。用于对世界上大多数书写系统中表示的文本进行一致的编码、表示和处理。`,
    answers: [
      `计算机科学领域`,
      `字符集`,
      `编码方案`,
...
010000-10FFFF   21    4     11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
  ...
  {
    id: 25,
    title: `为了弄清楚 UTF-16 文件的大小端字节序，所以在 UTF-16 文件的__，都会放置一个 U+FEFF 字符作为 BOM，以表示这个文本文件是用 UTF-16 编码的。如果一个文件开头的__个二进制的值转为十进制等于 65279（0xFEFF），则为__端字节序；等于 65534（0xFFFE），则为小端字节序。`,
    answers: ['开头', '16', '大'],
    tags: ['计算机科学', '编码'],
    type: '基础知识',
  },
