...
2. 经过层层分解，最后的子问题一定是一个不能再分解的__（即终止条件）。
  ...
  },
  {
    ...
    answers: [
      `AOP 全称“面向切面编程”是通过预编译方式和运行时动态代理方式实现在不修改源代码的情况下给程序动态添加功能的一种技术。`,
      {
        tag: 'pre',
        val:
          `
区别：
  ...
  },
  {
    ...
    answers: [
      `依赖注入，全称“依赖注入到容器”是将实例当变量传入到一个对象中去的技术。`,
      '背景：',
...
5. 接口隔离原则，是通过接口粒度大小来提高内聚性降低耦合性的原则。
    ...
    answers: [
      {
        ...
        val:
          `
定义：
  组合模式，是将对象组合成树状层次结构的拥有一致访问性的结构型设计模式。
优点：
    ...
    answers: [
      {
        ...
        val:
          `
定义：
  装饰模式是在不改变对象的情况下, 动态地为对象添加职责（功能）的结构型设计模式.
