...
<details>
  <summary>13.1-03 “DOM2级事件”规定的事件流包括 4 个阶段，哪 4 个阶段？</summary>
  <div>事件没有被处理阶段(0)、事件捕获阶段(1)、处于目标阶段(2)和事件冒泡阶段(3)</div>
</details>
...
<details>
  ...
  <div>3参：boolean；默认为false；false: 在冒泡阶段调用事件处理程序。true: 在捕获阶段调用事件处理程序</div>
</details>
...
<details>
  ...
  <div>- 背景: 添加到每一个DOM的事件处理程序都是对象都会占用内存 </div>
  <div>- 原理 : 事件冒泡</div>
</details>
